...
- **Week 9-10**: Cosmos Predict2.5 OMX 후훈련 + **IDM 시너지 파이프라인 구축** (병렬)
- **Week 11-12**: VLA 데이터 증강 재학습 + 세 모델 비교 분석 + 프로젝트 마무리
...
### Week 9-10: Cosmos Predict2.5 OMX 후훈련 + IDM 시너지 파이프라인 (병렬)

> **병렬 처리**: Week 9-10은 두 트랙을 병렬로 진행합니다.
> - **트랙 A**: Cosmos Predict2.5 Action-Conditioned OMX 후훈련 + 합성 비디오 생성
> - **트랙 B**: GR00T IDM pseudo labeling + VLA 데이터 증강 파이프라인 구축
>
> DreamDojo는 8x H100 + 증류 파이프라인 미공개로 인해, Cosmos Predict2.5가 세계 모델 역할을 대체합니다.
...
#### Week 10: 통합 파이프라인 실행 + 세 모델 크로스 분석
...
### Cosmos Predict2.5 + IDM 시너지 파이프라인 (권장)

DreamDojo를 Cosmos Predict2.5 Action-Conditioned로 대체한 파이프라인입니다.
코드가 완전 공개되어 있고, 1x RTX 4090에서 후훈련과 추론이 모두 가능합니다.
...
두 갈래로 활용 (병렬)
┌──────────────────────┬──────────────────────────┐
│ GR00T VLA 파인튜닝    │ FK 변환 → EE 포즈         │
│ (로봇 두뇌 학습)      │ → Cosmos Predict2.5 후훈련│
│ → OMX 배포/검증       │ (OMX 세계 모델 학습)      │
└──────────────────────┴──────────────────────────┘

Phase 2: 데이터 증강 (상상력으로 데이터 뻥튀기)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
기존 action에 변형 추가 (각도/속도/경로 변경)
    ↓
Cosmos Predict2.5 추론 → 합성 비디오 (수백~수천 회)
    ↓ resize (256×256) + 2-frame pairing
GR00T IDM 추론 → pseudo action label (IDM 관절 이름)
    ↓ 관절 이름 매핑 (IDM→VLA: shoulder_pan→joint1)
    ↓ 품질 평가 (jerk + temporal consistency → grade ≥ B)
    ↓ dtype 변환 (uint8→float32)
고품질 합성 데이터
...
| Cosmos Predict2.5 | 행동에 따른 미래 비디오 예측 (상상력) | 1x RTX 4090 |
| GR00T IDM | 비디오에서 행동 역추정 (관찰자) | 1x RTX 4090 |

**DreamDojo 대비 Cosmos Predict2.5의 장점:**

| 항목 | DreamDojo | Cosmos Predict2.5 |
|------|-----------|-------------------|
| 코드 공개 | 부분 공개 | 완전 공개 |
| 후훈련 GPU | 8x H100 80GB | 1x GPU 가능 |
| OMX 지원 | 미지원 (GR-1/G1/YAM) | 후훈련으로 지원 가능 |
| 증류 (실시간 추론) | 미공개 | 공개 (DMD2 가이드) |
| Custom action loader | 불가 | `action_load_fn` 커스터마이즈 가능 |
...
- **비용 절감**: 텔레오퍼레이션 대비 대규모 데이터 생성 가능 (1x GPU)
- **품질 보장**: IDM pseudo label 품질 평가로 저품질 데이터 필터링
- **완전 공개**: 코드/후훈련/증류 가이드 모두 공개
